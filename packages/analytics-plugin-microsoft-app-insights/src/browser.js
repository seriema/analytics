...
/**
 * Microsoft Application Insights plugin
 * @link https://getanalytics.io/plugins/microsoft-app-insights
 * @link https://github.com/Microsoft/ApplicationInsights-JS/
 ...
 * })
 */
function applicationInsightsPlugin(pluginConfig = {}) {
  // Allow for userland overides of base methods
  return {
    name: 'microsoft-app-insights',
    config: {
      ...config,
      ...pluginConfig
    },
    initialize: ({ config }) => {
      ...
      if (!containerId) {
        throw new Error('No Microsoft Application Insights containerId defined')
      }
      if (!scriptLoaded(containerId)) {
        ...
        script.src = `
          ...
          cfg: { // Application Insights Configuration
              instrumentationKey:${containerId}
          }});
        `;
...
export default applicationInsightsPlugin
...
function scriptLoaded(containerId) {
  ...
  if (!regex) {
    regex = new RegExp('applicationInsightsPlugin\\.com\\/gtm\\.js.*[?&]id=' + containerId)
